...
class BD
{
    ...
    {
        $conn = $this->conn();
        $sql = "INSERT INTO contato (nome, sobrenome, cpf, telefone1, tipo_telefone1, telefone2, tipo_telefone2, email) value (?, ?, ?, ?, ?, ?, ?, ?)";

        $st = $conn->prepare($sql);
        $arrayDados = [$dados['nome'], $dados['sobrenome'], $dados['cpf'], $dados['telefone1'], $dados['tipo_telefone1'], $dados['telefone2'],
    ...
    public function update($dados)
    {
        ...
        $dados['tipo_telefone2'],$dados['email'], $dados['id']];
        $st->execute($arrayDados);
